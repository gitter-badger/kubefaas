...
kubefaas fn create --name $fn --env $env --code $DIR/main_error.js --method GET

log "Creating message queue trigger"
kubefaas mqtrigger create --name $mqt --function $fn --mqtype "nats-streaming" --topic $topic --resptopic $resptopic --errortopic $errortopic --maxretries $maxretries
log "Updated mqtrigger list"
kubefaas mqtrigger list
...
response=$(timeout 30s go run $DIR/stan-sub/main.go --last -s $KUBEFAAS_NATS_STREAMING_URL -c $clusterID -id $subClientID $errortopic 2>&1 || true)
echo "$response"
