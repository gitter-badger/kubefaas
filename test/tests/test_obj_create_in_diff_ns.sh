...
cleanup() {
    [[ -n "${1+x}"  && -n "${2+x}" ]]; kubefaas env delete --name $1 --envns $2 || true
    [[ -n "${3+x}"  && -n "${4+x}" ]]; kubefaas fn delete --name $3 --fns $4 || true
    [[ -n "${5+x}"  && -n "${6+x}" ]]; kubefaas pkg delete --name $5 --pkgns $6 || true
    [[ -n "${7+x}"  && -n "${8+x}" ]]; kubefaas route delete --name $7 --triggerns $8 || true
}
...
internal_route_test_2() {
    http_status=`curl -sw "%{http_code}" http://$KUBEFAAS_ROUTER/kubefaas-function/func5 -o /tmp/file`
...
internal_route_test_1() {
    ns="ns2-$id"
    http_status=`curl -sw "%{http_code}" http://$KUBEFAAS_ROUTER/kubefaas-function/$ns/func4 -o /tmp/file`
    echo "http_status : $http_status"
    [[ "$http_status" -eq "200" ]]
}
...
new_deploy_mgr_and_internal_route_test_2() {
    ...
    kubefaas env create --name python --image kubefaas/python-env
    kubefaas fn create --name func5 --env python --code testDir1/hello.py --minscale 1 --maxscale 4 --executortype newdeploy
    sleep 15

    # function is loaded in $KUBEFAAS_NAMESPACE because func object was created in default ns
    ...
    internal_route_test_2 || ( log "internal route test for http://$KUBEFAAS_ROUTER/kubefaas-function/func5 returned http_status: $http_status" &&
    cleanup python "default" func5 "default" "" "" "" "" && exit 1)
...
new_deploy_mgr_and_internal_route_test_1() {
    ...
    kubefaas env create --name python --image kubefaas/python-env --envns "ns1-$id"
    kubefaas fn create --name func4 --fns "ns2-$id" --env python --envns "ns1-$id" --code testDir1/hello.py --minscale 1 --maxscale 4 --executortype newdeploy
    sleep 15
    ...
    internal_route_test_1 || (log "internal route test for http://$KUBEFAAS_ROUTER/kubefaas-function/ns2-$id/func4 returned http_status: $http_status" &&
    cleanup python "ns1-$id" func4 "ns2-$id" "" "" "" "" && exit 1)
...
builder_mgr_test_2() {
    ...
    kubefaas env create --name python-builder-env --builder kubefaas/python-builder --image kubefaas/python-env
    sleep 180

    # verify the env builder pod came up in kubefaasbuilder and env runtime pod came up in kubefaas-function ns
    verify_pod_ns python-builder-env kubefaas-builder || (log "python-builder-env builder env not found in kubefaas-builder ns" &&
    cleanup python-builder-env "default" "" "" "" "" "" "" && exit 1)
    verify_pod_ns python-builder-env $FUNCTION_NAMESPACE || (log "python-builder-env runtime env not found in kubefaas-function ns" &&
    cleanup python-builder-env "default" "" "" "" "" "" "" && exit 1)

    zip -jr src-pkg.zip $ROOT/examples/python/sourcepkg/
    pkg=$(kubefaas package create --src src-pkg.zip --env python-builder-env --buildcmd "./build.sh" --pkgns "ns2-$id"| cut -f2 -d' '| tr -d \')
    sleep 60
    kubefaas fn create --name func4 --fns "ns2-$id" --pkg $pkg --entrypoint "user.main"
    ht=$(kubefaas route create --function func4 --fns "ns2-$id" --url /func4 --method GET | cut -f2 -d' '| tr -d \')

    # get the function loaded into a pod
    sleep 10
    response=$(curl http://$KUBEFAAS_ROUTER/func4)
    echo $response
    ...
    # verify the function specialized pod is in kubefaas-function. This also verifies builder ran successfully and in kubefaas-builder
...
builder_mgr_test_1() {
    ...
    kubefaas env create --name python-builder-env --envns "ns1-$id" --builder kubefaas/python-builder --image kubefaas/python-env
    # we need to wait sufficiently for env pods to be up
    sleep 60

    zip -jr src-pkg.zip $ROOT/examples/python/sourcepkg/
    pkg=$(kubefaas package create --src src-pkg.zip --env python-builder-env --envns "ns1-$id" --buildcmd "./build.sh" --pkgns "ns2-$id"| cut -f2 -d' '| tr -d \')
    sleep 60
    kubefaas fn create --name func3 --fns "ns2-$id" --pkg $pkg --entrypoint "user.main"
    ht=$(kubefaas route create --function func3 --fns "ns2-$id" --url /func3 --method GET | cut -f2 -d' '| tr -d \')

    # get the function loaded into a pod
    sleep 10
    response=$(curl http://$KUBEFAAS_ROUTER/func3)
...
pool_mgr_test_2() {
    ...
    kubefaas env create --name python --image kubefaas/python-env
    kubefaas fn create --name func2 --fns "ns2-$id" --env python --code testDir1/hello.py
    ht=$(kubefaas route create --function func2 --fns "ns2-$id" --url /func2 | cut -f2 -d' '| tr -d \')

    # verify that env object is created in default ns when envns option is absent with kubefaas env create command
    ...
    sleep 3
    # get the function loaded into a pod
    response=$(curl http://$KUBEFAAS_ROUTER/func2)
...
pool_mgr_test_1() {
    ...
    kubefaas env create --name python --image kubefaas/python-env --envns "ns1-$id"
    kubefaas fn create --name func1 --fns "ns2-$id" --env python --envns "ns1-$id" --code testDir1/hello.py
    ht=$(kubefaas route create --function func1 --fns "ns2-$id" --url /func1 | cut -f2 -d' '| tr -d \')

    # verify that kubefaas objects are created in the expected namespaces
    ...
    sleep 3
    # get the function loaded into a pod
    response=$(curl http://$KUBEFAAS_ROUTER/func1)
    echo $response | grep "Hello" || (log "response Hello not received" &&
    cleanup python "ns1-$id" func1 "ns2-$id" $pkg "ns2-$id" $ht "ns2-$id" && exit 1)
...
main() {
    ...
    # this test and namespaces wont clash when kubefaas CI tests are run in parallel in the future.
    id=`echo $KUBEFAAS_NAMESPACE| cut -d"-" -f2`
    ...
    #    route and verify specialized pod in kubefaas-builder. this test is to verify backward compatibility
    builder_mgr_test_2
    ...
    # 2. env default, func ns2 with code, route and verify specialized pod in kubefaas-function
    new_deploy_mgr_and_internal_route_test_2

    # internal route tests. ( combined this with new deploy mgr tests, so we dont have to recreate envs and funcs again )
    # 1. env ns1, func ns2 with code, curl http://KUBEFAAS_ROUTER/kubefaas-function/ns2/func -> should work
    # 2. env in default, func ns2 with code, curl http://KUBEFAAS_ROUTER/kubefaas-function/func -> should work


    # TODO : add following tests
    # timer trigger tests.
    # 1. env ns1, func ns2 with code, tt ( with a one time cron string for executing imm'ly), verify function is executed
    # this also indirectly tests internal route established at http://KUBEFAAS_ROUTER/ns1/func

    # 2. env in default, func ns2 with code, tt ( with a one time cron string for executing imm'ly), verify function is executed
    # this also indirectly tests internal route established at http://KUBEFAAS_ROUTER/func
