...
func GetFissionNamespace() string {
	fissionNamespace := os.Getenv("KUBEFAAS_NAMESPACE")
	return fissionNamespace
}

func GetApplicationUrl(selector string) (string, error) {
	var serverUrl string
	// Use KUBEFAAS_URL env variable if set; otherwise, port-forward to controller.
	fissionUrl := os.Getenv("KUBEFAAS_URL")
	if len(fissionUrl) == 0 {
...
func GetVersion(client client.Interface) info.Versions {
	...
	if err != nil {
		console.Warn(fmt.Sprintf("Error getting API version: %v", err))
		serverInfo = &info.ServerInfo{}
	}

	// Fetch server versions
	versions.Server = map[string]info.BuildMeta{
		"kubefaas/core": serverInfo.Build,
	}
...
func GetServerURL(input cli.Input) (serverUrl string, err error) {
	...
	if len(serverUrl) == 0 {
		...
		serverUrl, err = GetApplicationUrl("application=kubefaas-api")
		if err != nil {
			return "", err
		}
	}
