...
func runPortForward(labelSelector string, localPort string, ns string) error {
	...
	if len(podList.Items) > 0 {
		...
		if len(nsList) > 1 {
			return errors.Errorf("Found %v kubefaas installs, set KUBEFAAS_NAMESPACE to one of: %v",
	...
	if !ok {
		return errors.Errorf("Error finding kubefaas install within the given namespace %v, please check KUBEFAAS_NAMESPACE is set properly", ns)
	}
	...
	if err != nil {
		return errors.Errorf("Failed to connect to Kubefaas service on Kubernetes")
	}
