...
import (
	...
	storageSvcClient "github.com/srcmesh/kubefaas/pkg/storagesvc/client"
...
func UploadArchiveFile(ctx context.Context, client client.Interface, fileName string) (*fv1.Archive, error) {
	...
	if size < fv1.ArchiveLiteralSizeLimit {
		...
		storageSvc, err := client.V1().Misc().GetSvcURL("application=kubefaas-storage")
		storageSvcURL := "http://" + storageSvc
		if err != nil {
			return nil, errors.Wrapf(err, "error getting kubefaas storage service name")
		}
