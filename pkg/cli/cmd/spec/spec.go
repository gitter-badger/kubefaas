...
import (
	...
	"github.com/srcmesh/kubefaas/pkg/cli/cmd/spec/types"
...
const (
	DEPLOYMENT_NAME_KEY = "kubefaas-name"
	DEPLOYMENT_UID_KEY  = "kubefaas-uid"
...
func save(data []byte, specDir string, specFile string) error {
	// verify
	if _, err := os.Stat(filepath.Join(specDir, "kubefaas-deployment-config.yaml")); os.IsNotExist(err) {
		return errors.Wrap(err, "Couldn't find specs, run `kubefaas spec init` first")
	}
...
// called from `kubefaas * create --spec`
...
// the right list in the given kubefaas resources set.
