...
import (
	...
	pkgutil "github.com/srcmesh/kubefaas/pkg/cli/cmd/package/util"
	spectypes "github.com/srcmesh/kubefaas/pkg/cli/cmd/spec/types"
...
// applyResources applies the given set of kubefaas resources.
...
func localArchiveFromSpec(specDir string, aus *spectypes.ArchiveUploadSpec) (*fv1.Archive, error) {
	...
	if len(files) > 1 || !isSingleFile {
		...
		archiveFile, err := ioutil.TempFile("", fmt.Sprintf("kubefaas-archive-%v", aus.Name))
		if err != nil {
			return nil, err
		}
...
func applyDeploymentConfig(m *metav1.ObjectMeta, fr *FissionResources) {
	...
	m.Annotations[DEPLOYMENT_NAME_KEY] = fr.DeploymentConfig.Name
	m.Annotations[DEPLOYMENT_UID_KEY] = fr.DeploymentConfig.UID
}
...
func hasDeploymentConfig(m *metav1.ObjectMeta, fr *FissionResources) bool {
	...
	uid, ok := m.Annotations[DEPLOYMENT_UID_KEY]
