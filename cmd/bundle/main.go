...
import (
	...
	"github.com/srcmesh/kubefaas/cmd/bundle/mqtrigger"
...
func registerTraceExporter(logger *zap.Logger, arguments map[string]interface{}) error {
	...
	serviceName := "Kubefaas-Unknown"

	if arguments["--controllerPort"] != nil {
		serviceName = "Kubefaas-Controller"
	} else if arguments["--routerPort"] != nil {
		serviceName = "Kubefaas-Router"
	} else if arguments["--executorPort"] != nil {
		serviceName = "Kubefaas-Executor"
	} else if arguments["--kubewatcher"] == true {
		serviceName = "Kubefaas-KubeWatcher"
	} else if arguments["--timer"] == true {
		serviceName = "Kubefaas-Timer"
	} else if arguments["--mqt"] == true {
		serviceName = "Kubefaas-MessageQueueTrigger"
	} else if arguments["--builderMgr"] == true {
		serviceName = "Kubefaas-BuilderMgr"
	} else if arguments["--storageServicePort"] != nil {
		serviceName = "Kubefaas-StorageSvc"
	}
...
func main() {
	...
	usage := `bundle: Package of all kubefaas microservices: controller, router, executor.

Use it to start one or more of the kubefaas servers:

 Controller is a stateless API frontend for kubefaas resources.
...
Usage:
  bundle --controllerPort=<port>
  bundle --routerPort=<port> [--executorUrl=<url>]
  bundle --executorPort=<port> [--namespace=<namespace>] [--kubefaas-namespace=<namespace>]
  bundle --kubewatcher [--routerUrl=<url>]
  bundle --storageServicePort=<port> --filePath=<filePath>
  bundle --builderMgr [--storageSvcUrl=<url>] [--envbuilder-namespace=<namespace>]
  bundle --timer [--routerUrl=<url>]
  bundle --mqt   [--routerUrl=<url>]
  bundle --logger
  bundle --version
Options:
  ...
  --namespace=<namespace>         Kubernetes namespace in which to run function containers. Defaults to 'kubefaas-function'.
...
`
	...
	version := fmt.Sprintf("Bundle Version: %v", info.BuildInfo().String())
	...
	functionNs := getStringArgWithDefault(arguments["--namespace"], "kubefaas-function")
	envBuilderNs := getStringArgWithDefault(arguments["--envbuilder-namespace"], "kubefaas-builder")

	executorUrl := getStringArgWithDefault(arguments["--executorUrl"], "http://executor.kubefaas")
	routerUrl := getStringArgWithDefault(arguments["--routerUrl"], "http://router.kubefaas")
	storageSvcUrl := getStringArgWithDefault(arguments["--storageSvcUrl"], "http://storagesvc.kubefaas")
